...
function ArtworkTab({
  ...
  // Deco artwork from orders
  const [decoArtwork, setDecoArtwork] = useState<DecoArtworkItem[]>([]);
  const [loadingDeco, setLoadingDeco] = useState(false);
  const [decoExpanded, setDecoExpanded] = useState(true);
  ...
  // Fetch Deco artwork from orders
  useEffect(() => {
    if (!accountId) return;
    setLoadingDeco(true);
  ...
  return (
    <div className="space-y-4">
      ...
      {accountId && (
        <div>
          <button onClick={() => setDecoExpanded(!decoExpanded)} className="flex w-full items-center justify-between">
            <SectionHeader>
              Deco Artwork
              {decoArtwork.length > 0 && (
            ...
            <svg className="h-3.5 w-3.5 transition-transform"
              style={{ color: "var(--text-tertiary, #64748b)", transform: decoExpanded ? "rotate(0deg)" : "rotate(-90deg)" }}
          ...
          {decoExpanded && (
            <div className="mt-2">
              {loadingDeco ? (
                <p className="text-[11px] py-3 text-center" style={{ color: "var(--text-tertiary, #64748b)" }}>Loading Deco artwork...</p>
              ) : decoArtwork.length === 0 ? (
                <p className="text-[11px] py-3 text-center" style={{ color: "var(--text-tertiary, #64748b)" }}>No artwork found in Deco orders</p>
              ) : (
                <div className="grid grid-cols-2 gap-2">
                  {decoArtwork.map((art) => {
